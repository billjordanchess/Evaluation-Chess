...
void Gen(const int s,const int xs)
{
	...
	while(b1)
	{
	  ...
	  b1 &= not_mask[sq];
	  n = pawnleft[s][sq];
	  AddCapture(sq,n,matrix[0][board[n]]);
	}
	while(b2)
	{
      sq = NextBit(b2);
...
while(b1)
{
	sq = NextBit(b1);
	b1 &= not_mask[sq];
	b2 = bit_moves[N][sq] & bit_units[xs];
	while(b2)
	{
		n = NextBit(b2);
		AddCapture(sq,n,matrix[N][board[n]]);
		b2 &= not_mask[n];
	}
	b2 = bit_moves[N][sq] & ~bit_all;
	while(b2)
	{
		n = NextBit(b2);
		AddMove(sq,n);
...
int sq2;
for(int piece=2;piece<=4;piece++)
{
b1 = bit_pieces[s][piece];
while(b1)
{
	sq = NextBit(b1);
	b1 &= not_mask[sq];
	b2 = bit_moves[piece][sq];
	b3 = bit_units[s];
	while(b3 & b2)
	{
		sq2 = NextBit(b2 & b3);
	...
	while(b2 & b3)
	{
		...
		{
			AddCapture(sq,sq2,matrix[piece][board[sq2]]);
		}
		b2 &= bit_after[sq][sq2];
	}
	while(b2)
...
b1 = bit_moves[K][sq] & bit_units[xs];
while(b1)
{
	n = NextBit(b1);
	AddCapture(sq,n,matrix[K][board[n]]);
	b1 &= not_mask[n];
}
b1 = bit_moves[K][sq] & ~bit_all;
while(b1)
{
	n = NextBit(b1);
	AddMove(sq,n);
...
while(b1)
{
	...
	{
		sq2 = NextBit(b2);
		AddCapture(sq,sq2,matrix[1][board[sq2]]);
		b2 &= not_mask[sq2];
	}
	b1 &= not_mask[sq];
}
...
{
	sq = NextBit(b1);
	b2 = bit_moves[piece][sq] & bit_units[xs];
	while(b2)
	{
		sq2 = NextBit(b2);
		if(!(bit_between[sq][sq2] & bit_all))
...
sq = NextBit(bit_pieces[s][K]);
b2 = bit_moves[K][sq] & bit_units[xs];
while(b2)
{
	sq2 = NextBit(b2);
	AddCapture(sq,sq2,matrix[5][board[sq2]]);
	b2 &= not_mask[sq2];
}
first_move[ply + 1] = mc;
}
